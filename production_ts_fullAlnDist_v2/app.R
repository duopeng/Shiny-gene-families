...
library(stringr)
library(shiny)
loc = read.delim(paste0("data/ts_BrA4_YC6_fullAlnDist_v2",".fasta.aln.fasta.aln2.fasta.mat.loc.tab"), col.names=c("gene_ID","chr","start","end","strand","annotation"), header = FALSE, sep = "\t", quote = "\"", dec = ".", fill = TRUE)
rownames(loc) <- loc[,1]
...
ui <- fluidPage(
  ...
  '),
  fluidRow( column(width=4, selectInput("gFamily", "Select gene family:", selected="ts_BrA4_YC6_fullAlnDist_v2", choices = c("ts_BrA4_YC6_fullAlnDist_v2"))),
  ...
  fluidRow(column(width=10, textAreaInput("geneList", "Showing only genes in the list below: (one gene per line)", value = "all", width = 400,
                                          height = 100, cols = NULL, rows = NULL, placeholder = "all",
                                          resize = NULL)
  ),
  ),            
  
  fluidRow(column(width=3, actionButton("applybutton", "Click to apply settings"))),
  fluidRow(column(width=3, downloadButton("downloadbutton", "Click to download plot data"))),
...
server <- function(input, output) {
  ...
  progress$inc(1/4, detail = paste("Loading required R packages"))
  
  library(ggplot2)
  library(RColorBrewer)
  library(stringr)
  
  progress$inc(1/4, detail = paste("Loading precomputed distance matrix"))
  
  #load distance matrix
  dat = read.delim(paste0("data/ts_BrA4_YC6_fullAlnDist_v2",".aln.fasta.aln2.fasta.mat"), header = FALSE, sep = " ", quote = "\"", dec = ".", fill = TRUE)
  dat2 <- dat[,-1]
  rownames(dat2) <- dat[,1]
  colnames(dat2) <- dat[,1]
  
  progress$inc(1/5, detail = paste("Loading gene annotations"))
  ...
  progress$inc(1/5, detail = paste("Calculating MDS projection"))
  #MDS
  d <- as.dist(dat2)
  mds.coor <- cmdscale(d)
  ...
  output$distPlot <- renderPlot({
    isolate({
    ggplot(plot_mat, aes_string('x', 'y', color='specie',shape='specie')) + 
      geom_point()+
      scale_shape_manual(values=c(1,4))+
      labs(title="MDS representation of pairwise genetic distances",
  ...
  output$downloadbutton <- downloadHandler(
    filename = function() {
      paste('data-', input$gFamily, '.csv', sep='')
    },
    content = function(con) {
      write.csv(plot_mat[,-1], con)
    }
  )
  ...
  observeEvent(input$applybutton,{
    ...
    #load distance matrix
    dat = read.delim(paste0("data/",input$gFamily,".aln.fasta.aln2.fasta.mat"), header = FALSE, sep = " ", quote = "\"", dec = ".", fill = TRUE)
    dat2 <- dat[,-1]
    rownames(dat2) <- dat[,1]
    colnames(dat2) <- dat[,1]
    ...
    #load loc
    loc = read.delim(paste0("data/",input$gFamily,".fasta.aln.fasta.aln2.fasta.mat.loc.tab"), col.names=c("gene_ID","chr","start","end","strand","annotation"), header = FALSE, sep = "\t", quote = "\"", dec = ".", fill = TRUE)
    rownames(loc) <- loc[,1]
    loc$length=loc$end-loc$start
    loc$geneID_contig_length_annotation=paste(loc$gene_ID,loc$chr,loc$length,loc$annotation,sep=' ') 
    loc$specie = str_split_fixed(loc$gene_ID, "_",2)[,1]
    ...
    progress$inc(1.5/5, detail = paste("performing MDS"))
