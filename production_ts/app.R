...
loc = read.delim(paste0("data/ts_BrA4_YC6",".fasta.mat.loc.tab"), col.names=c("gene_ID","chr","start","end","strand","annotation"), header = FALSE, sep = "\t", quote = "\"", dec = ".", fill = TRUE)
rownames(loc) <- loc[,1]
...
ui <- fluidPage(
  ...
  '),
  fluidRow( column(width=4, selectInput("gFamily", "Select gene family:", selected="ts_BrA4_YC6", choices = c("ts_BrA4_YC6"))),
...
server <- function(input, output) {
  ...
  dat = read.delim(paste0("data/ts_BrA4_YC6",".fasta.mat"), header = FALSE, sep = " ", quote = "\"", dec = ".", fill = TRUE)
  dat2 <- dat[,-1]
  rownames(dat2) <- dat[,1]
  colnames(dat2) <- dat[,1]
