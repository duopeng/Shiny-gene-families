...
ui <- fluidPage(
  ...
  '),
  fluidRow( column(width=4, selectInput("gFamily", "Select gene family:", selected="ts_BrA4_YC6_fullAlnDist", choices = c("ts_BrA4_YC6_fullAlnDist"))),
...
server <- function(input, output) {
  ...
  observeEvent(input$applybutton,{
    ...
    #load distance matrix
    dat = read.delim(paste0("data/",input$gFamily,".fasta.aln.fasta.mat"), header = FALSE, sep = " ", quote = "\"", dec = ".", fill = TRUE)
    dat2 <- dat[,-1]
    rownames(dat2) <- dat[,1]
    colnames(dat2) <- dat[,1]
    ...
    #load loc
    loc = read.delim(paste0("data/",input$gFamily,".fasta.aln.fasta.mat.loc.tab"), col.names=c("gene_ID","chr","start","end","strand","annotation"), header = FALSE, sep = "\t", quote = "\"", dec = ".", fill = TRUE)
    rownames(loc) <- loc[,1]
    loc$length=loc$end-loc$start
    loc$geneID_contig_length_annotation=paste(loc$gene_ID,loc$chr,loc$length,loc$annotation,sep=' ') 
    loc$specie = str_split_fixed(loc$gene_ID, "_",2)[,1]
